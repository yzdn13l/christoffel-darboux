...
# from metrics.PyTorchEMD2.emd_nograd import earth_mover_distance_nograd as EMD
# from metrics.PyTorchEMD2.emd_static import earth_mover_distance as EMD
...
def _pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size, accelerated_cd=True):
    ...
    iterator = tqdm(range(N_sample), desc='Computing EMD and CD', total=N_sample)
...
def _mp_pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size, accelerated_cd=True):
    N_sample = sample_pcs.shape[0]
    N_ref = ref_pcs.shape[0]
    all_cd = []
    all_emd = []

    def process_sample(sample_b_start):
        sample_batch = sample_pcs[sample_b_start]
        cd_lst = []
        emd_lst = []
        for ref_b_start in range(0, N_ref, batch_size):
        ...
        cd_lst = torch.cat(cd_lst, dim=1)
        emd_lst = torch.cat(emd_lst, dim=1)
        return sample_b_start, cd_lst, emd_lst

    with ThreadPoolExecutor(max_workers=8) as executor:
        results = list(tqdm(executor.map(process_sample, range(N_sample)), desc='Computing EMD and CD', total=N_sample))

    # shuffle the results back to the original order
    results = sorted(results, key=lambda x: x[0])
    # print(results[:10])
    for i, cd_lst, emd_lst in results:
        all_cd.append(cd_lst)
        all_emd.append(emd_lst)
...
def compute_all_metrics(sample_pcs, ref_pcs, batch_size):
    ...
    print("Computing EMD and CD between sample and ref")
    ...
    print("Computing EMD and CD between ref and ref")
    M_rr_cd, M_rr_emd = _pairwise_EMD_CD_(ref_pcs, ref_pcs, batch_size)
    print("Computing EMD and CD between sample and sample")
...
def compute_all_metrics_mp(sample_pcs, ref_pcs, batch_size):
    results = {}

    print("Computing EMD and CD between sample and ref")
    M_rs_cd, M_rs_emd = _mp_pairwise_EMD_CD_(ref_pcs, sample_pcs, batch_size)
    ...
    print("Computing EMD and CD between ref and ref")
    M_rr_cd, M_rr_emd = _mp_pairwise_EMD_CD_(ref_pcs, ref_pcs, batch_size)
    print("Computing EMD and CD between sample and sample")
    M_ss_cd, M_ss_emd = _mp_pairwise_EMD_CD_(sample_pcs, sample_pcs, batch_size)
