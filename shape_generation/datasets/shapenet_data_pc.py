...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
                 ...
                 normalize_globally=True, box_globally=False,
        ...
        self.box_globally = box_globally
        ...
        if all_points_mean is not None and all_points_std is not None:  # using loaded dataset stats
            ...
            self.all_points_mean = (self.all_points.min(axis=1).reshape(B, 1, input_dim) + self.all_points.max(axis=1).reshape(B, 1, input_dim)) / 2
            self.all_points_std = (self.all_points.max(axis=1).reshape(B, 1, input_dim) - self.all_points.min(axis=1).reshape(B, 1, input_dim)) / 2
        elif self.normalize_globally:  # normalize across the dataset
        ...
        elif self.box_globally:
            self.all_points_mean = self.all_points.mean(axis=(0, 1)).reshape(1, 1, input_dim)
            self.all_points_std = np.sqrt(((self.all_points - self.all_points_mean)**2).sum(axis=-1)).max().reshape(1, 1, 1)
            print("Box globally")
        else:
            raise ValueError("Normalization not specified")
        
        self.all_points = (self.all_points - self.all_points_mean) / self.all_points_std
        # print("All points mean: ", self.all_points_mean)
        # print("All points std: ", self.all_points_std)
        # print("All points min: ", self.all_points.min())
        # print("All points max: ", self.all_points.max())
    ...
    def cache_fps_points(self, num_fps_points=1024):
        self.train_points = torch.from_numpy(self.train_points).float() # (N, 10000, 3)
        self.test_points = torch.from_numpy(self.test_points).float() # (N, 5000, 3)
        new_train_points = []
        new_test_points = []
        # for i in tqdm(np.arange(0, len(self.train_points), 256), desc="Caching FPS Train"):
        for i in np.arange(0, len(self.train_points), 256):
            new_train_points.append(sample_farthest_points(self.train_points[i:i+256].cuda(), K=num_fps_points)[0])
            # new_train_points.append(farthest_point_sampling(self.train_points[i:i+256].cuda(), num_fps_points))
        for i in np.arange(0, len(self.test_points), 256):
            new_test_points.append(sample_farthest_points(self.test_points[i:i+256].cuda(), K=num_fps_points)[0])
            # new_test_points.append(farthest_point_sampling(self.test_points[i:i+256].cuda(), num_fps_points))
        self.train_points = torch.cat(new_train_points, dim=0).cpu().numpy()
        self.test_points = torch.cat(new_test_points, dim=0).cpu().numpy()
        self.tr_sample_size = num_fps_points
        self.te_sample_size = num_fps_points
...
class ShapeNet15kPointClouds(Uniform15KPC):
    def __init__(self, root_dir="data/ShapeNetCore.v2.PC15k",
        ...
        super(ShapeNet15kPointClouds, self).__init__(
            ...
            normalize_globally=normalize_globally, box_globally=box_globally,
