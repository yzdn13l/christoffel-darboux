...
def parse_args():
    ...
    parser.add_argument('--model', default='PointNetCls', help='model name [default: pointnet_cls]')
    parser.add_argument('--loss', default='cross_entropy', help='loss function')
...
def main():
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        train_out = train(net, train_loader, optimizer, args.loss, device)  # {"loss", "acc", "acc_avg", "time"}
        test_out = validate(net, test_loader, args.loss, device)
        scheduler.step()
...
def train(net, trainloader, optimizer, loss_type, device):
    net.train()
    train_loss = 0
    correct = 0
    total = 0
    ...
    for batch_idx, (data, label) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        logits = net(data)
        loss, logits = net.get_loss(data, label, loss_type)
        loss.backward()
...
def validate(net, testloader, loss_type, device):
    net.eval()
    test_loss = 0
    correct = 0
    total = 0
